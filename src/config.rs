...
impl Program {
    ...
    pub fn get_args(&self) -> &HashMap<String, String> { &self.args }
    ...
    pub fn get_icon_on_path(&self) -> Option<&str> { self.ui.icons.on.as_deref() }

    pub fn get_icon_off_path(&self) -> Option<&str> { self.ui.icons.off.as_deref() }
...
fn replace_args(str: &String, args: &HashMap<String, String>) -> String {
    ...
    let result = re.replace_all(&str, |caps: &regex::Captures| {
        ...
        args.get(&arg_name).cloned().unwrap_or_else(|| format!("${}", arg_name))
    });
...
fn parse_content(content: &str) -> io::Result<Program> {
    match toml::from_str(&content) {
        ...
        Err(error) => {
            Err(io::Error::new(ErrorKind::InvalidInput, error.message()))
        }
    }
}
